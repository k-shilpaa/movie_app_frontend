...
export class UserService {
  ...
  //   return this.http.post<any>(`${this.userUrl2}/forgot-password?email=${email}`, null);
  // }
  ...
  //   return this.http.post(this.userUrl2 + '/resetPassword', data).pipe(
  //     map((response: any) => {
  //       console.log('Response from server:', response);
  //       if (response && response.statusCode === 'OK') {
  //         return response.body; // Successful response
  //       } else {
  //         throw new Error(response.body || 'Password updation failed.');
  //       }
  //     }),
  //     catchError((error) => {
